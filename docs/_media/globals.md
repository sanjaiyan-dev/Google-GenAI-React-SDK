[**react-google-genai**](README.md)

***

# react-google-genai

The intelligence layer for your UI. A type-safe React SDK for `@google/genai` for static content generation with TanStack-powered mutations, sub-second latency, and optimized state management ✨

## Interfaces

- [ChatMessage](interfaces/ChatMessage.md)
- [GenerateContentResponse](interfaces/GenerateContentResponse.md)
- [GenerateResult](interfaces/GenerateResult.md)
- [GoogleGenAI](interfaces/GoogleGenAI.md)
- [Part](interfaces/Part.md)
- [StreamState](interfaces/StreamState.md)
- [Tool](interfaces/Tool.md)
- [UseChatOptions](interfaces/UseChatOptions.md)
- [UseFunctionCallingOptions](interfaces/UseFunctionCallingOptions.md)
- [UseGenerateContentOptions](interfaces/UseGenerateContentOptions.md)
- [UseGenerateContentOptionsQuery](interfaces/UseGenerateContentOptionsQuery.md)
- [UseInteractionBaseCreateHookMutate](interfaces/UseInteractionBaseCreateHookMutate.md)
- [UseInteractionBaseCreateHookQuery](interfaces/UseInteractionBaseCreateHookQuery.md)
- [UseInteractionBaseGetQuery](interfaces/UseInteractionBaseGetQuery.md)
- [UseStreamContentHook](interfaces/UseStreamContentHook.md)
- [UseStreamContentOptions](interfaces/UseStreamContentOptions.md)
- [UseStreamContentQueryOptions](interfaces/UseStreamContentQueryOptions.md)

## Type Aliases

- [ChatRole](type-aliases/ChatRole.md)
- [FunctionHandler](type-aliases/FunctionHandler.md)
- [GenAIProviderConfig](type-aliases/GenAIProviderConfig.md)

## Functions

- [extractText](functions/extractText.md)
- [extractTextFromParts](functions/extractTextFromParts.md)
- [GenAIProvider](functions/GenAIProvider.md)
- [imagePrompt](functions/imagePrompt.md)
- [textPrompt](functions/textPrompt.md)
- [useChat](functions/useChat.md)
- [useFunctionCalling](functions/useFunctionCalling.md)
- [useGenAIClient](functions/useGenAIClient.md)
- [useGenerateContentMutate](functions/useGenerateContentMutate.md)
- [useGenerateContentQuery](functions/useGenerateContentQuery.md)
- [useInteractionBaseCreateMutate](functions/useInteractionBaseCreateMutate.md)
- [useInteractionBaseCreateQuery](functions/useInteractionBaseCreateQuery.md)
- [useInteractionBaseGetQuery](functions/useInteractionBaseGetQuery.md)
- [useModelInfo](functions/useModelInfo.md)
- [useStreamContent](functions/useStreamContent.md)
- [useStreamContentQuery](functions/useStreamContentQuery.md)
